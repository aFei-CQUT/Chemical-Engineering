...
# 加载 Excel 文件
ExcelFile = pd.ExcelFile(r'./流体原始数据记录表(非).xlsx')

# 获取工作表名称
sheet_names = ExcelFile.sheet_names

def fluid_flow_analysis(file_dir):
    
    """
    进行流体流动分析，包括计算雷诺数和摩擦系数，并进行双对数拟合。

    Parameters:
        file_dir (str): Excel 文件路径，包含流体数据。
    ...
    # 从第一个工作表中读取数据 
    df = pd.read_excel(file_dir, sheet_name=sheet_names[0], header=None)
    
    # 提取特定数据
    
    # 流量数据(m^3/s)
    Q = df.iloc[2:, 1].values/3600/1000
    
    # 压力降数据(Pa)
    ΔPf = pd.concat([1000 * df.iloc[2:11, 2],  ρ * g * df.iloc[11:, 3]/1000], ignore_index=True).values
    ...
    plt.grid(True)
    plt.legend(loc='best')
    plt.savefig('./拟合图结果/雷诺数与阻力系数双对数拟合(无插值)', dpi=300)
    ...
    plt.grid(True)
    plt.legend(loc='best')
    plt.savefig('./拟合图结果/雷诺数与阻力系数双对数拟合(有插值)', dpi=300)

    plt.show()

    return np.column_stack((u, Re, λ)), df

ans1, df1 = fluid_flow_analysis(file_dir)
...
def centrifugal_pump_characteristics(file_dir):
    
    """
    分析离心泵的特性曲线，包括扬程、功率和效率的计算与二次拟合。

    Parameters:
    file_dir (str): Excel 文件路径，包含离心泵特性数据。
    ...
    # 从第二个工作表中读取数据
    df = pd.read_excel(file_dir, sheet_name=sheet_names[1], header=None)
...
# =============================================================================
    ...
    # 对泵的扬程进行二次拟合
    params_H, _ = curve_fit(quadratic, Q, H)
    H_fit = quadratic(Q, *params_H)

    # 对泵的有效功率进行二次拟合
    params_N_elc_e, _ = curve_fit(quadratic, Q, N_elc_e)
    N_fit = quadratic(Q, *params_N_elc_e)

    # 对泵的效率进行二次拟合
    params_η, _ = curve_fit(quadratic, Q, η)
    η_fit = quadratic(Q, *params_η)
...
ans2,df2 = centrifugal_pump_characteristics(file_dir)
